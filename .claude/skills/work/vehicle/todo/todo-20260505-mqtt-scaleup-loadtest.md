...
- [x] 서버 정상 동작 확인: `curl http://localhost:8081/api/cartracking/vehicles` (1000대 응답 확인)
- [x] HiveMQ CE 정상 동작 확인: `docker ps`
- [x] VisualVM에서 서버 JVM 연결 확인
- [x] JMeter에서 간단한 1회 요청 성공 확인
- [x] 시뮬레이터 count 파라미터 동작 확인: `curl -X POST 'http://localhost:8081/api/cartracking/simulator/start?count=10'`
...
| 단계 | 차량 수 | MQTT msg/s | JMeter REST | Duration | 뭘 보는가 |
|------|--------|-----------|-------------|----------|----------|
| S1 | 10대 | 2/s | 10 threads, GET /vehicles | 60s | baseline: 모든 지표 정상인가? |
| S2 | 50대 | 10/s | 30 threads, GET /vehicles | 60s | 50대로 늘려도 괜찮은가? |
| S3 | 100대 | 20/s | 50 threads, GET /vehicles | 60s | broadcast 지연이 시작되는가? |
| S4 | 500대 | 100/s | 50 threads, GET /vehicles | 60s | 어디서 병목이 터지나? |
| S5 | 1000대 | 200/s | 50 threads, GET /vehicles | 60s | 극한: 서버가 버티는가? |

> **핵심 변수는 차량 수(= 메시지 빈도)**이다.
> REST 동시 사용자 수는 고정 또는 소폭 증가만 시킨다.
> JMeter WebSocket 스레드 그룹은 duration 모드에서 hang 발생하여 비활성화 (ADR-V032).
> WebSocket broadcast 성능은 서버 내부 PipelineMetrics로 측정한다.

#### 각 단계 실행 절차

```
자동화 스크립트: bash loadtest-run.sh
(아래 절차를 S1~S5 순차 실행, 각 스테이지 간 Enter로 대기)

1. 시뮬레이터 시작 (N대)
   curl -X POST 'http://localhost:8081/api/cartracking/simulator/start?count=N'

2. 10초 대기 (워밍업)

3. JMeter 실행 (시뮬레이터가 돌아가는 중에, 60초 duration)
   - REST Thread Group: GET /api/cartracking/vehicles
   - WebSocket Thread Group: 비활성화 (ADR-V032)

4. JMeter 종료 후 결과 기록
   - REST: Avg, p99, TPS, Error%
   - PipelineMetrics: MQTT/REST 구간별 avg/p99/max (서버 perf 로그)
   - VisualVM: 힙 사용량, GC 횟수, 스레드 수

5. 시뮬레이터 중지 → Enter 눌러 다음 단계로
   (서버 재시작 후 진행)
```
...
```
| 단계 | 차량 수 | REST Avg | REST p99 | REST Max | REST TPS | REST Error% | 총 요청 |
|------|--------|---------|---------|---------|---------|------------|---------|
| S1   | 10     | 4ms     | 54ms    | 545ms   | 2,494   | 0%         | 148,760 |
| S2   | 50     | 11ms    | 96ms    | 2,658ms | 2,711   | 0%         | 162,219 |
| S3   | 100    | 18ms    | 263ms   | 2,197ms | 2,632   | 0%         | 157,594 |
| S4   | 500    | 17ms    | 266ms   | 989ms   | 2,881   | 0%         | 170,252 |
| S5   | 1000   | 17ms    | 253ms   | 992ms   | 2,800   | 0%         | 165,953 |

> **측정일시**: 2026-05-10 22:57 ~ 23:29 KST
>
> **조건**: Keep-Alive 적용 완료(ADR-V029), WebSocket sampler 비활성화(ADR-V032), duration 60s
>
> **결과 요약**:
> - 에러 0% — 1000대까지 서버가 전부 처리
> - TPS 거의 일정 (~2,500~2,800) — 차량 수 증가에도 TPS 미하락
> - Avg 응답시간: S1(4ms) → S3~S5(17~18ms) — 10대→100대 구간에서 증가 후 안정화
> - p99: S1(54ms) → S3~S5(253~266ms) — 꼬리 지연 약 5배 증가
> - Avg/p99 증가 원인: GET /vehicles 응답 JSON 크기 증가 (10대 목록 vs 1000대 목록 직렬화)
> - **결론: InMemory 기반에서는 1000대까지 병목 없이 처리 가능**
>
> **JMeter HTML 리포트**: `D:/apache-jmeter-5.6.3/result_cartracking/S1~S5/index.html`
```
...
#### ~~개선 1: HTTP Keep-Alive 지원~~ ✅ 완료 (2026-05-10, ADR-V029)
- **변경**: `HttpUtil.isKeepAlive(request)`로 판단, keep-alive 시 연결 유지
- **결과**: SocketException 50% 에러 해소
...
- ~~HTTP Keep-Alive 미지원~~ → 지원 완료 (ADR-V029)
