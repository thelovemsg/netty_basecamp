...
### 한 마디로
**가짜 차량이 5초마다 GPS 좌표를 만들어서 HiveMQ CE 브로커에 보내는 시뮬레이터를 만들었다.**

### 구체적으로 만든 것

```
docker-compose.yml       ← HiveMQ CE 브로커를 Docker로 띄우는 설정
...
```
1. docker compose up -d              → HiveMQ CE 브로커 시작 (port 1883)
...
| **HiveMQ CE** | 브로커 구현체 (Docker) | "CJ대한통운" — 택배 회사 |
...
**브로커 전략:**
- 지금은 HiveMQ CE(기성품)를 Docker로 띄워서 사용
...
HiveMQ CE는 별도 설정 파일 없이 기본값으로 동작한다 (익명 접속 허용, port 1883).
